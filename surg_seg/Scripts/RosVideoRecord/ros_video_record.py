...
import signal
from surg_seg.Utils.Logger import Logger

log = Logger("VideoRecorder", "INFO").log
...
class Sync:
    def __init__(self, path_package, config):
        ...
        if self.is_otf_compression_on:
            self.set_up_otf_compression(
                config["on_the_fly_lossless_compression"]["format_opti"],
        ...
        self.ats = message_filters.ApproximateTimeSynchronizer(
            [self.data_sub_im1, self.data_sub_im2], queue_size=500, slop=1.0 / self.fps
        )
    ...
    def set_up_otf_compression(self, frmt_opti, n_cores_requested, counter_between_runs):
        ...
        self.frame_frmt_opti = (
            frmt_opti  # Convert `.bmp` to `frmt_opti` in separate processes, to save memory
        )
        ...
        )  # Max we need 10 cores, since there are 10 numbers in total
        n_cores_max = mp.cpu_count()
        self.n_cores = min(
            n_cores, n_cores_max
        )  # We cannot request more cores than what the computer is equipped with
    ...
    def set_up_output_dir(self, path_package, out_dir):
        ...
        if not os.path.isdir(out_dir):
            os.makedirs(out_dir)
            log.info("Directory {} created".format(out_dir))
    ...
    def set_up_ffmpeg_command(self, codec, crf):
        ...
        self.ffmpeg_command = [
            "ffmpeg",
            "-r",
            "{}".format(self.fps),  # Frames per second
            "-i",
            "{}_%{}{}".format(self.out, self.frame_pad, frame_frmt),  # Input images
            "-c:v",
            "{}".format(codec),  # Video codec
            "-b:v",
            "0",  # Set bitrate to 0, to adjust quality from the "crf" value
            "-crf",
            "{}".format(crf),  # Constant quality encoding (the lower the better the quality)
            "-strict",
            "experimental",  # To allow experimental codecs as well
            "-an",  # No audio
            "{}".format(self.out_vid),
        ]  # Output video path
    ...
    def sync_callback(self, msg_im1, msg_im2):
        ...
        """ Save image """
        frame_path = "{}_{:{}}{}".format(
            self.out, self.counter_save, self.frame_pad, self.frame_frmt
        )
    ...
    def start_processes_to_compress_otf(self):
        ...
        for ends in self.ends_splitted:
            ...
            self.pool.apply_async(
                compress_frames,
                args=(frame_path, self.frame_frmt, self.frame_frmt_opti),
                callback=self.log_result,
            )

    def stop_recording_and_compress_video(self):
        self.is_shutdown = True
        log.info("Stopped recording new frames!")
        ...
        frame_frmt = self.frame_frmt
        if self.is_otf_compression_on:
            log.info("Please wait until all the individual frames are compressed...")
            while not self.frame_compress_completed:
                time.sleep(3)
            log.info("Running the last compression of frames... It may take a while, please wait")
            self.start_processes_to_compress_otf()
            self.pool.close()
            self.pool.join()
            frame_frmt = self.frame_frmt_opti
        ...
        if path_im_list:
            log.info("Compressing recorded images into video. Please wait...")
            if self.is_vid_on:
                ...
                if self.discard_individual_frames:
                    log.info("Discarding individual frames...")
...
def compress_frames(frame_path, frame_frmt, frame_frmt_opti):
    ...
    )  # Make it ignore Ctrl + C, not to interfere with ROS Ctrl + C
    ...
    while not rospy.is_shutdown():
        ...
        if sync.is_visualization_on:
            if sync.frame is not None:
                cv.imshow(window_name, sync.frame)
